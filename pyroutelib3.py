...
class Datastore:
    ...
    def getArea(self, lat, lon):
        ...
        directory = os.path.join("tilescache")
    ...
    def getRouteWidth(self,Type):
        if Type == "secondary":
            return 7/12742/6*1.1
        elif Type == "residential":
            return 3.5/12742/6*1.1
        elif Type == "footway":
            return 1/12742/6*1.1
        elif Type == "cycleway":
            return 3/12742/6*1.1
        elif Type == "pedestrian":
            return 3/12742/6*1.1
        elif Type == "tertiary":
            return 5/12742/6*1.1
        elif Type == "service":
            return 2/12742/6*1.1
        elif Type == "path":
            return 1/12742/6*1.1
        else:
            return 0.0001/12742/6*1.1
...
class Router(Datastore):
    ...
    def getNumberOfNodesBetweenThose(self, number, nodeNow, nodeNext):
        allNodes = []
        Y2 = nodeNext[1]
        X2 = nodeNext[0]
        Y1 = nodeNow[1]
        ...
        for i in range(1,number,1):
          Y3 = Y1 + i*L/number * math.sin(Theta)
          X3 = X1 + i*L/number * math.cos(Theta)
          allNodes.append([i,X3,Y3])

    

        return allNodes

    def getNodeWay(self, NodeId1, NodeId2):    
        for i in self.ways:
            for nd1 in self.ways[i]["nd"]:
                if nd1 == NodeId1:
                    for nd2 in self.ways[i]["nd"]:
                        
                        if nd2 == NodeId2:
                            #print("The nd1 :%d and nd2 : %d and Node1: %d and Node2: %d" % (nd1,nd2,NodeId1,NodeId2))
                            return self.ways[i]

    def getOneNodeWay(self, NodeId1):    
        for i in self.ways:
            for nd1 in self.ways[i]["nd"]:
                if nd1 == NodeId1:
                    return self.ways[i]
                
    def getLatLongWithNewWidth(self, width,PathLength, nodeNow,nodeNext):
        ...
        DX_AB = (BX - AX)
        Delta = -0.2
        lamda = math.atan2(DY_AB,DX_AB)+Delta
        DY_CA = AC * math.cos(lamda)
        DX_CA = AC * math.sin(lamda)
        Xc = AX + DX_CA
        Yc = AY - DY_CA
        nodePoint = [Yc,Xc]
